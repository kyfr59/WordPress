...
if (!class_exists('ReduxFramework_select_label')) {
    ...
    class ReduxFramework_select_label extends ReduxFramework {
            ...
            if (isset($this->value) && is_array($this->value)) {
                ...
                if ($this->field['options']) {
                    foreach ($this->field['options'] as $opt_key => $opt_value) {
                        ...
                        if(empty($selected_key)) $selected_key = 'not-selected';
                        
                        echo '<input type="hidden" id="' . $this->field['id'] . '-' . $selected_key . '" name="' . $this->field['name'] . '[' . $opt_key . ']' . $this->field['name_suffix'] . '" value="' . esc_attr($this->value[$opt_key]) . '" class="regular-text value-store ' . $this->field['class'] . '" />';

                        echo '</li>';
                    }
                } else {
                    echo __('Please set primary Menu.', 'pi_framework');
                }
        ...
        public function enqueue() {
            ...
            wp_enqueue_script(
                    'redux-field-menu_manager-js', TEMPLATEURL . '/core/theme-options/custom-fields/select_label/field_select_label.js', array('jquery'), time(), true
            );

            wp_enqueue_style(
                    'redux-field-menu_manager-css', TEMPLATEURL . '/core/theme-options/custom-fields/select_label/field_select_label.css', time(), true
            );
        }

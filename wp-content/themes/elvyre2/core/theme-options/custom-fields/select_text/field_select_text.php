...
if (!class_exists('ReduxFramework_select_text')) {
    ...
    class ReduxFramework_select_text extends ReduxFramework {
        ...
        public function render() {
            ...
            if (isset($this->value) && is_array($this->value)) {
                ...
                foreach ($this->value['icon'] as $k => $value) {
                    ...
                    echo '<input type="text" id="' . $this->field['id'] . '-' . $k . '" name="' . $this->field['name'] . '[text][]' . $this->field['name_suffix'] . '" value="' . esc_attr($this->value['text'][$k]) . '" class="regular-text ' . $this->field['class'] . '" /> <a href="javascript:void(0);" class="deletion redux-select-text-remove">' . __('Remove', 'redux-framework') . '</a>';
                    echo '</li>';
                }
                ...
                echo '</select>';
                echo '<input type="text" id="' . $this->field['id'] . '" name="' . $this->field['name'] . '[text][]' . $this->field['name_suffix'] . '" value="" class="regular-text ' . $this->field['class'] . '" /> <a href="javascript:void(0);" class="deletion redux-select-text-remove" placeholder="http://">' . __('Remove', 'redux-framework') . '</a>';
                echo '</li>';
            }
        ...
        public function enqueue() {
            ...
            wp_enqueue_script(
                    'redux-field-select_text-js', TEMPLATEURL . '/core/theme-options/custom-fields/select_text/field_select_text.js', array('jquery'), time(), true
            );

            wp_enqueue_style(
                    'redux-field-select_text-css', TEMPLATEURL . '/core/theme-options/custom-fields/select_text/field_select_text.css', time(), true
            );
        }
